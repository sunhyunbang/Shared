...
AS
  ...
  IF (SELECT COUNT(*) FROM PP_LOGO_OPTION_TB WHERE LINEADNO = @LINEADNO) > 0
    BEGIN -- 수정
      ...
      IF (SELECT COUNT(*) FROM PP_LOGO_OPTION_TB WHERE LINEADNO = @LINEADNO AND ISNULL(WAIT_LOGO, '') <> @LOGO_IMAGE) > 0 
        BEGIN
          IF dbo.FN_GET_OPTINFO_YN(@LINEADID,@INPUT_BRANCH,@LAYOUT_BRANCH) ='Y' --홈상품일경우 재수정/검수대기 상태로 처리 21.09.29
            BEGIN
