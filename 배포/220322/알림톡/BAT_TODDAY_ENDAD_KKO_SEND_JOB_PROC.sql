...
AS  
 ...
 SELECT A.LINEADID  
  ...
  SELECT 1 AS [STATUS]  
        ...
        ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@MSG,'#{회사명}', COM_NM),'#{제목}',TITLE),'#{광고번호}',LINEADID), '#{공고제목}',TITLE),'#{공고번호}',LINEADID),'#{등록일}',REG_DT),'#{YYYY-MM-DD ~ YYYY-MM-DD}',CONVERT(VARCHAR(10), START_DT ,120) + ' ~ ' +CONVERT(VARCHAR(10), END_DT ,120)),'#{day}',DATEDIFF(D,START_DT,END_DT))   AS FAILED_MSG  
        ,'FINDALL' AS ETC1  
    FROM #MMS_TMP_TB  
