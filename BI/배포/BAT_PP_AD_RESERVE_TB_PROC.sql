 ...
 */  
 ...
 ,MODIFY_DT =GETDATE()  
 --select *  
 FROM PP_AD_RESERVE_TB A WITH(NOLOCK) INNER JOIN PP_AD_TB B WITH(NOLOCK) ON A.LINEADNO = B.LINEADNO  
 where (B.DELYN = 'Y' OR B.ENDYN='Y')  
 AND RESERVE_STAT ='예약'  
 OPTION(MAXDOP 1)  
 ...
 IF @CUID<> 0 AND @KKAO_SEND_YN ='Y' BEGIN  
  INSERT #KAKAO_SEND_TB (LINEADID, TITLE, REG_DT, START_DT, END_DT, GROUP_CD, CUID, [VERSION], MAG_ID, OPTCODE_NAME, OPT_START_DT, OPT_END_DT )  
  SELECT @LINEADID, @TITLE, @REG_DT, @RESERVE_START_DT, @RESERVE_END_DT, @GROUP_CD ,@CUID ,@VERSION ,@MAG_ID, @OPTCODE_NAME, @OPT_START_DT, @OPT_END_DT  
 ...
 BEGIN  
  ...
  FOR  
   SELECT  
    LINEADID, TITLE, REG_DT, START_DT, END_DT, GROUP_CD,PHONE, COM_NM, [VERSION], BRANCH_NAME, BRANCH_PHONE, OPTCODE_NAME, OPT_START_DT, OPT_END_DT  
   FROM #KAKAO_SEND_TB  
  OPEN KAKAO_CURSOR  
